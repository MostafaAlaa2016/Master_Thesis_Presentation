...
\begin{tikzpicture}[scale=.9]
  ...
  \node at (2.3, \yStartA +\half) {$=$};

  % Rectangle C
  \draw[rounded corners=2pt]  (\xStartC-.5, \yStartC) rectangle (\xEndC-.5, \yEndC)
  ...
  % Annotation
  \node at (\xStartC -\shiftMargin-.5, \yStartA +\half) {\small 41};
  \draw[arrows=-angle 90, very thin]  (\xStartC -\shiftMargin-.5, 2.5) -- (\xStartC -\shiftMargin-.5, \yEndC -0.1);
  \draw[arrows=angle 90-, very thin]  (\xStartC -\shiftMargin-.5, 0.5) -- (\xStartC -\shiftMargin-.5, 2);

  \draw [decorate,decoration={brace,amplitude=2pt,mirror,raise=2pt}, very thin]
  (\xEndC +\shiftMargin -.1-.7, 2.5 +.2) -- (\xEndC +\shiftMargin  -.1-.7, \yEndC -0.1);
%  node [right, midway, xshift=0.2cm, yshift=0.1cm] {\tiny $4 \times 1$ diacritic vector};

  \draw [decorate,decoration={brace,amplitude=2pt,mirror,raise=2pt}, very thin]
  (\xEndC +\shiftMargin -.1-.7, 0.5) -- (\xEndC +\shiftMargin -.1-.7, 2.4);
%  node [right, midway, xshift=0.2cm, yshift=0.1cm] {\tiny $37 \times 1$ letter vector};


  \node [rectangle, text width= 4em,font=\footnotesize] at (4.7, 3.4)
    {\scriptsize $4 \times 1$ vector represents letter \textarabic{◌َ}};

  \node [rectangle, text width= 4em,font=\footnotesize] at (4.7, 1.4)
    {\scriptsize $37 \times 1$ vector represents letter \textarabic{ب}};
